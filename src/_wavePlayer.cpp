...
static void _loadWavFiles(File dir, int numTabs);
static bool _hasWavFileExt(char* filename);
...
static bool _playWavFromSD(const char* filename)
{
  char fname[80];
...
bool _add_to_playlist(char *filename)
{
	int i;
...
static void _loadWavFiles(File dir, int numTabs) {
  ...
  while(true)
  {
    ...
    if ( _hasWavFileExt(tempc) && file_name.indexOf('_') != 0 )
	{ // Here is the magic
